...
";
        ...
        public async Task<Project> AddProjectAsync(Project project, IUserContext? userContext)
        {
            ...
            var query =
$@"INSERT INTO Projects (Name, Description, Location, Color, UserId, DateCreatedUtc, DateModifiedUtc) 
VALUES(@Name, @Description, @Location, @Color, @UserId, @DateCreatedUtc, @DateModifiedUtc);
";
...
LIMIT {request.Results} OFFSET {offsetRecords};";
        ...
        public async Task<StoredFile> UpdateStoredFileAsync(StoredFile storedFile, IUserContext? userContext)
        {
            ...
            {
                throw new StorageProviderException(nameof(SqliteStorageProvider), $"Record not found for {nameof(StoredFile)} = {storedFile.StoredFileId}");
            }
            return storedFile;
        }
...
";
        ...
        public async Task<Pcb> UpdatePcbAsync(Pcb pcb, IUserContext? userContext)
        {
            ...
            {
                throw new StorageProviderException(nameof(SqliteStorageProvider), $"Record not found for {nameof(Pcb)} = {pcb.PcbId}");
            }
            return pcb;
        }
...
";
        ...
        public async Task<PcbStoredFileAssignment> UpdatePcbStoredFileAssignmentAsync(PcbStoredFileAssignment assignment, IUserContext? userContext)
        {
            ...
            {
                throw new StorageProviderException(nameof(SqliteStorageProvider), $"Record not found for {nameof(PcbStoredFileAssignment)} = {assignment.PcbStoredFileAssignmentId}");
            }
            return assignment;
        }
...
";
        ...
        public async Task<ProjectPartAssignment> UpdateProjectPartAssignmentAsync(ProjectPartAssignment assignment, IUserContext? userContext)
        {
            ...
            {
                throw new StorageProviderException(nameof(SqliteStorageProvider), $"Record not found for {nameof(ProjectPartAssignment)} = {assignment.ProjectPartAssignmentId}");
            }
            return assignment;
        }
...
";
        ...
        public async Task<ProjectPcbAssignment> UpdateProjectPcbAssignmentAsync(ProjectPcbAssignment assignment, IUserContext? userContext)
        {
            ...
            {
                throw new StorageProviderException(nameof(SqliteStorageProvider), $"Record not found for {nameof(ProjectPcbAssignment)} = {assignment.ProjectPcbAssignmentId}");
            }
            return assignment;
        }
